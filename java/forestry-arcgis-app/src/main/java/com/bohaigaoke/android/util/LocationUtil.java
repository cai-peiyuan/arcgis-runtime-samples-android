...
public class LocationUtil {
	...
	/**
	 * 判断GPS导航是否打开. false：弹窗提示打开,不建议采用在后台强行开启的方式。 true:不做任何处理
	 * 
	 * @return
	 */
	...
	public void formListenerGetLocation() {
		...
		/**
		 * 绑定监听 参数1，设备：有GPS_PROVIDER和NETWORK_PROVIDER两种，前者是GPS,后者是GPRS以及WIFI定位
		 * 参数2，位置信息更新周期.单位是毫秒 参数3，位置变化最小距离：当位置距离变化超过此值时，将更新位置信息 参数4，监听
	...
	/**
	 * 主动获取Location，通过以下方法获取到的是最后一次定位信息。 注意：Location location=new
		...
		GpsStatus.Listener listener = new GpsStatus.Listener() {
			...
			public void onGpsStatusChanged(int event) {
				if (event == GpsStatus.GPS_EVENT_FIRST_FIX) {
					...
					Iterator<GpsSatellite> it = gpsStauts.getSatellites()
